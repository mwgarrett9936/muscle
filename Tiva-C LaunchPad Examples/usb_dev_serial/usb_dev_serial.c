...
ReadUARTData(void)
{
    ...
    //Do ADC, convert to chars, and put in UART buffer
    //
    //NOT USB NOT USB NOT USB
    ...
    sprintf(buffer, "%u", ulADC_Value[0]);
    sprintf(buffer2, "%u", ulADC_Value[1]);
    ...
    while(ulSpace && ROM_UARTCharsAvail(USB_UART_BASE))
    {
        ...
        if(!(lChar & ~0xFF))
        {
            ...
            //USBBufferWrite((tUSBBuffer *)&g_sTxBuffer,
             //              (unsigned char *)&ucChar, 1); //orginal
            //ADD THIS TO SEND BACK UART ASCII
            //ucChar='x';
           // ROM_UARTCharPutNonBlocking(USB_UART_BASE, ' ');
            //ROM_UARTCharPutNonBlocking(USB_UART_BASE,' ');
            int i;
            for(i=0;i<4;i++){
            ROM_UARTCharPutNonBlocking(USB_UART_BASE,buffer[i]);
            }
            ROM_UARTCharPutNonBlocking(USB_UART_BASE,';');
            for(i=0;i<4;i++){
            ROM_UARTCharPutNonBlocking(USB_UART_BASE,buffer2[i]);
            }
            //
...
USBUARTPrimeTransmit(unsigned long ulBase)
{
    ...
    // Trigger the ADC conversion.
     ...
     ADCIntClear(ADC_BASE, 0);
        ...
        //convert to string of chars
         char buffer[5];
         //sprintf(buffer, "%ld", ulADC_Value[0]);
    ...
    while(ROM_UARTSpaceAvail(ulBase))
    {
        ...
        if(ulRead)
        {
        	...
        	ucChar = 'r';
            //
            //ROM_UARTCharPutNonBlocking(ulBase, ucChar); //original
        	//Write back using USB NOT UART
        	 //int i=0;
        	// for(i=0;i<3;i++){
        	   //ROM_UARTCharPutNonBlocking(USB_UART_BASE, buffer[i]);
        		 USBBufferWrite((tUSBBuffer *)&g_sTxBuffer,
...
main(void)
{
   ...
   // ADCSequenceDisable(ADC0_BASE,1);
    ...
    ADCSequenceStepConfigure(ADC_BASE, 0, 0, ADC_CTL_CH8);
    ADCSequenceStepConfigure(ADC_BASE, 0, 1, ADC_CTL_CH11|ADC_CTL_IE |
            ADC_CTL_END);

    //ADCSequenceStepConfigure(ADC0_BASE, 2, 2, ADC_CTL_CH11);
    ...
    //ADCSequenceStepConfigure(ADC1_BASE, 2, 0, ADC_CTL_CH8 | ADC_CTL_IE |
    //	                             ADC_CTL_END);
